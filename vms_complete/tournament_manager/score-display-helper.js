...
function getSportScoringConfig(sportName) {
  const SPORT_CONFIGS = {
    ...
    'chess': {
      ...
      results: {
        '1-0': { label: 'White Wins', color: '#1f2937' },
        '½-½': { label: 'Draw', color: '#6b7280' },
        '0-1': { label: 'Black Wins', color: '#1f2937' }
      }
    }
  };
...
/**
 * Parse Chess score string back to components
 * Format: "1-0 (42 moves) [Rapid]"
 */
function parseChessScore(scoreString) {
  ...
  return {
    ...
    moves: movesMatch ? parseInt(movesMatch[1]) : null,
    timeFormat: formatMatch ? formatMatch[1] : null
  };
}
 ...
 * @param {string} score - The raw score from database
...
function formatScoreWithLabels(score, sportName, detailed = true) {
  ...
  if (config.type === 'result') {
    ...
    const resultInfo = config.results[parsed.result] || { label: parsed.result, color: '#6b7280' };
    
    if (!detailed) {
      return `<div style="display: flex; align-items: center; gap: 4px;">
        <strong style="font-size: 16px; color: ${resultInfo.color};">${parsed.result}</strong>
        ${parsed.moves ? `<span style="font-size: 10px; color: #9ca3af;">${parsed.moves}m</span>` : ''}
      </div>`;
    }
    ...
    let html = `
      <div style="display: flex; flex-direction: column; gap: 6px;">
        <div style="display: flex; align-items: center; gap: 8px;">
          <span style="
            background: ${resultInfo.color === '#1f2937' ? 'linear-gradient(135deg, #111827 0%, #374151 100%)' : 'linear-gradient(135deg, #9ca3af 0%, #d1d5db 100%)'};
            color: white;
            padding: 6px 12px;
            border-radius: 6px;
            font-size: 14px;
    ...
    if (parsed.timeFormat) {
      const formatColors = {
        'Blitz': { bg: '#fef3c7', color: '#92400e', icon: '⚡' },
        'Rapid': { bg: '#dbeafe', color: '#1e40af', icon: '⏱️' },
        'Classical': { bg: '#f3e8ff', color: '#6b21a8', icon: '♟️' }
      };
      const formatStyle = formatColors[parsed.timeFormat] || formatColors['Rapid'];
      
      html += `
        <div style="display: flex; align-items: center; gap: 6px;">
          <span style="
            background: ${formatStyle.bg};
            ...
            font-weight: 600;
          ">
            ${formatStyle.icon} ${parsed.timeFormat}
          </span>
        </div>
      `;
    }
  ...
  if (score.includes('(')) {
    ...
    if (parts) {
      ...
      let html = `<div><strong>Total: ${total}</strong></div>`;
      html += '<div style="font-size: 11px; color: #6b7280; margin-top: 4px;">';
      
      scores.forEach((s, idx) => {
        if (s && s.trim() !== '0') {
          const label = config.labels[idx] || `Part ${idx + 1}`;
          html += `<span style="margin-right: 8px;">${label}: ${s}</span>`;
        }
      });
  ...
  if (config.type === 'sets' || config.type === 'quarters' || config.type === 'halves' || config.type === 'innings') {
    ...
    scores.forEach((s, idx) => {
      if (s && s.trim() !== '' && s.trim() !== '0') {
        const label = config.labels[idx] || `${config.type === 'innings' ? 'Inning' : 'Set'} ${idx + 1}`;
        html += `
          <div style="font-size: 11px;">
            <span style="color: #6b7280; min-width: 60px; display: inline-block;">${label}:</span>
...
function getScoreBreakdown(score, sportName) {
  ...
  if (config.type === 'result') {
    ...
    breakdown.push({ 
      label: 'Game Result', 
      value: `${parsed.result} (${resultInfo.label})`, 
      isTotal: true,
      highlight: true,
...
function createScoreCard(scoreData) {
  ...
  let sportBadge = '';
  if (config.type === 'components') {
    sportBadge = '<span style="background: #fef3c7; color: #92400e; padding: 2px 6px; border-radius: 3px; font-size: 10px; font-weight: 600;">DANCE SPORT</span>';
  } else if (config.type === 'result') {
    sportBadge = '<span style="background: #111827; color: white; padding: 2px 6px; border-radius: 3px; font-size: 10px; font-weight: 600;">♟️ CHESS</span>';
  }
      ...
      html += `
        <div style="display: flex; justify-content: space-between; padding: 4px 0; font-size: 12px;">
          ...
          <strong style="
            ...
            ${item.highlight ? 'background: #f3f4f6; padding: 2px 8px; border-radius: 4px;' : ''}
          ">
            ${item.value}
          </strong>
        </div>
...
if (typeof window !== 'undefined') {
  ...
  window.parseChessScore = parseChessScore;
